...
class Renewables extends CI_Controller {
...
//			$avatar=$this->media_model->getmediainfo($avatar->avatar_media_id,"media_url");
	//		$data['profle_pic']=$avatar->media_url;
		...
		$data['Renewables']=$this->Renewables_model->all();
		$data['Clients']=$this->Clients_model->all();
		$data['free_service_expiry']=$this->Renewables_model->free_service_expiry();
		$data['expires_7']=$this->last_sentdate(7);
		$data['expires_15']=$this->last_sentdate(15);
		$data['expires_30']=$this->last_sentdate(30);
		$data['expired']=$this->last_sentdate(0);
	...
	public function last_sentdate($days){
		$haids=$this->Renewables_model->expires($days);
		if($haids!=null){
			foreach ($haids as $info) {
				$date=$this->Notifications_model->last($info->client_ID, $info->ID);
				if($date!=null) { $info->sent_date=$date[0]->sent_date; }
			}
			return $haids;
		}
	}
	...
	{
		$data=$this->common();
		$data['title']="Renewables";
		if($this->input->post('add_account')!=null){
			...
			$this->Renewables_model->add($inputs);
			redirect("Renewables");
		}

		$this->load->view('Renewables',$data);
	}
